...
def _create_test_snapshot(snapshot_path: Path) -> None:
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO agents (id, project_id, name) VALUES (2, 1, 'Bob')"
        )
        ...
        conn.execute(
            "INSERT INTO message_recipients (message_id, agent_id, kind) VALUES (2, 1, 'to')"
        )
...
def test_build_materialized_views_basic(tmp_path: Path):
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT id, sender_name FROM message_overview_mv WHERE id = 1"
        )
        row = cursor.fetchone()
        assert row[1] == "Alice"
...
def test_build_materialized_views_with_recipients(tmp_path: Path):
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT id, recipients FROM message_overview_mv WHERE id = 1"
        )
        row = cursor.fetchone()
...
def test_attachments_materialized_view(tmp_path: Path):
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO projects (id, slug) VALUES (1, 'test')"
        )
        conn.execute(
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT message_id, attachment_type, media_type FROM attachments_by_message_mv"
        )
...
def test_case_insensitive_search_optimization(tmp_path: Path):
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT id, subject FROM messages WHERE subject_lower LIKE '%urgent%' ORDER BY id"
        )
        rows = cursor.fetchall()
