...
async def ensure_archive_if_enabled(settings: Settings, project: Project) -> ProjectArchive | None:
    """Return the project archive when enabled; otherwise, skip archive work."""
                ...
                async with get_session() as session:
                    ...
                    if agent:
                        ...
                        if not getattr(agent, "is_placeholder", False):
                            ...
                            await session.commit()
                            await session.refresh(agent)
                            archive = await ensure_archive_if_enabled(settings, project)
                            if archive:
...
async def _delete_agent(project: Project, name: str, settings: Settings) -> dict[str, Any]:
    ...
    5. Writes a deletion marker to the Git archive when archive storage is enabled
...
def build_mcp_server() -> FastMCP:
    ...
    async def ensure_project(ctx: Context, human_key: str) -> dict[str, Any]:
        ...
        - Safe to call multiple times. If the project already exists, the existing
          record is returned and, when enabled, the archive is ensured on disk (no
          destructive changes).
        """
        await ctx.info(f"Ensuring project for key '{human_key}'.")
        project = await _ensure_project(human_key)
        await ensure_archive_if_enabled(settings, project)
    ...
    async def register_agent(
        ...
        """
        Create or update an agent identity within a project and persist its profile to
        Git when archive storage is enabled.
        ...
        - When archive storage is enabled, a `profile.json` file is written under
          `agents/<Name>/` in the project archive. If archive is disabled, registration
          still succeeds but Git persistence is skipped.
